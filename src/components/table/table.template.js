...
function toCell(state, row) {
  return function (_, col) {
    ...
    return `<div class="cell" contenteditable  data-value="${data || ''}" data-col="${col}" data-id="${id}" data-type="cell" style="${styles}; width: ${width}">${
      parse(data) || ''
    }</div>`;
  };
}
...
function toColumn({ col, index, width }) {
  return `
    <div class="column" data-type="resizаble" data-col="${index}" style="width: ${width}">
      ${col}
...
function createRow(index, content, state) {
  ...
  return `
        <div class="row" data-type="resizаble" data-row="${index}" style="height: ${height}">
